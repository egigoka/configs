...
# configs: resolve this script's own dir so the override file is found regardless
# of the caller's working directory (upstream assumed cwd == the cloned repo).
...
# configs: vendored alongside this script as tailscale-override.conf
cp -f "$SCRIPT_DIR/tailscale-override.conf" /etc/systemd/system/tailscaled.service.d/override.conf
