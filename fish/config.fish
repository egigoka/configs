if status is-interactive
  ...
  abbr --add ga --position command git add
  abbr --add gcl --position command git clone
  ...
  if string match -q "Darwin*" -- (uname)
    ...
    # Keep unlocked SSH key copies only until the Mac locks, sleeps, logs out,
    # or restarts. Hammerspoon and a LaunchAgent remove this directory.
    function ssh --wraps ssh --description "SSH with temporary unlocked key copies"
      _ssh_with_temporary_unlocked_keys $argv
    end

    function mosh --wraps mosh --description "Mosh with temporary unlocked SSH key copies"
      set -l mosh_ssh "fish -c '_ssh_with_temporary_unlocked_keys \$argv' --"
      command mosh --ssh="$mosh_ssh" $argv
    end
