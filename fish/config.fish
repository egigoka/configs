if status is-interactive
  ...
  switch (uname -s)
    ...
    case Darwin
      # mAcos
      if test (id -u) -eq 0
        abbr --add updateall --position command fisher update \&\& sudo git -C /opt/macports-wine pull \&\& sudo port selfupdate \&\& sudo port upgrade outdated \&\& sudo port reclaim --enable-reminders \&\& sudo ~/configs/install_scripts/autopatch_qbittorrent_macports.sh
      else
